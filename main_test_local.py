...
class Reseau2neurone_RELU :
   ...
   def backward_propagation(self,label): #on va d'abord commencer à calculer les erreurs pour les neurones
       ...
       L = self.nb_couche
       sortie = self.activation[L-1]
       somme_sortie = self.sommes[L-1]
       erreur = 2*(sortie-vecteur_attendu)*np.where(somme_sortie > 0, 1, 0)
       self.erreurs[L-1]=erreur #on a trouvé l'erreur de la couche de sortie, puis on va la projeter
       #vers les couches inférieures, avec la formule mathématique correspondante
       ...
       for couche in range(self.nb_couche-1, 0, -1):
           ...
           gradient = np.outer(self.erreurs[couche],activation_avec_biais)  # calcule de gradiant pour tout les poids de la couche
